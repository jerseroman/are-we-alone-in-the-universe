...
13 in the zero branch. Consequently, the direct local empirical-support
criterion is \emph{not met}, even though the geometry and code checks pass.
...
For the temperature-matched G-star column (5300--6000 K), Table~5 of
\citet{Bryson2021} reports Model~1 conservative-HZ occurrence of
$0.38^{+0.50}_{-0.22}$ planets per star for the constant-extrapolation low
bound and $0.63^{+0.94}_{-0.38}$ for the zero-extrapolation high bound. The
reconstructed v4 broad-HZ medians are 0.388 and 0.659. This close
temperature-matched agreement supports source fidelity, but it is not
independent validation: the present posterior is reconstructed from the same
public DR25 model and inputs. Bryson et al.'s 0.37 and 0.60 results for the
broader 4800--6300 K $\eta_\oplus$ temperature range remain useful context but
are not the primary source-fidelity benchmark here.
