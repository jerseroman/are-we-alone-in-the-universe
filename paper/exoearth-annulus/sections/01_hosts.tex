...
$R_{\rm TAMS}(T)$ is the frozen public \texttt{tams\_parsec.txt} boundary from the Berger/Huber evolutionary-state implementation \citep{Berger2018,HuberEvolstate}. The exact upstream table is pinned at commit \hashid{5e904afad81805c4e3ac4c3f78510a2a1df33d14}; the verbatim project copy has SHA-256 \hashid{d2c47b264a298a599064a9e58f19f309886e7b96f36cc9603c9ca55494f87aac}. Its generator uses solar-composition PARSEC v1.2S tracks ($Z=0.017$, $Y=0.279$), reconstructs radius from luminosity and temperature, and records the first \texttt{PHASE==7} point below 20 Gyr. Within the present target range the eight required tabulated nodes span 5200--6060.24246 K, are monotonic in temperature, and are interpolated linearly in $T$ and $\log_{10}(R_{\rm TAMS}/R_\odot)$; extrapolation is forbidden. Appendix~\ref{app:tams} lists the adopted nodes.

This is a temperature-dependent evolutionary boundary, not the fixed $R_\star<1.35R_\odot$ cut rejected in the source analysis. It is used as a transport approximation to the Berger/Huber \texttt{Evol} classification inherited by the Bryson denominator, not as a claim that the complete Kepler and JJ stellar selections are identical. The one-dimensional boundary has no explicit metallicity coordinate; a differential metallicity-dependent alternative is included in Section~\ref{sec:sensitivity}.
...
\end{align}
with $\Delta R=0.5$ kpc. Figure~\ref{fig:surface_density} shows the selected-host surface density across the full JJ radial range.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{host_surface_density.pdf}
\caption{Surface density of selected old G-star hosts in the thin and thick JJ disks. The shaded 7--9 kpc interval is the estimand domain. The integrated thick-disk contribution within this annulus is 19.9\%; the node-by-node fraction ranges from 19.0\% to 21.8\%.}
